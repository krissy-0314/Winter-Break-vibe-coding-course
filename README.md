# 寒假 Vibe Coding 課程資料整理

**課程名稱：** 從開發工具到產品思維  
**講者：** Adam & Hao / 七號演算股份有限公司

---

## 課程資源總覽

| 週次 | 單元主題 | 簡報 | HackMD | 影片 |
|:----:|----------|:----:|:------:|:----:|
| Week 1 | Vibe Coding 入門 | ✅ | ✅ | ✅ |
| Week 2 | Web 小工具快速上線 | ✅ | ✅ | ✅ |
| Week 3 | GAS 介紹與應用 | ❌ | ✅ | ✅ |
| Week 4 | LINE Bot 助理 | ✅ | ✅ | ✅ |
| Week 5 | 爬蟲技術介紹 | ❌ | ✅ | 待放 |

---

## Week 1｜單元一：Vibe Coding 入門

### 📋 課程主題
**打造你的第一個 AI 協作專案** — 拆解問題 + 環境建置 + 驗收思維

### 🎯 學習重點
- 學會將模糊需求拆解為 IPO（輸入 → 處理 → 輸出）
- Vibe Coding 核心：不是拼命寫 Code，而是精準提問與驗收
- 黃金法則 3×3：3 個成功案例 + 3 個失敗案例
- AI 時代開發迴圈：Prompt → Generate → Verify → Iterate

### 📎 課程資源

| 項目 | 連結 |
|------|------|
| 📝 操作手冊 | [HackMD 連結 1](https://hackmd.io/@vVHC1jffQWuFZd6HVu7sFg/ryEiQGN4Wx) |
| 📝 操作手冊 | [HackMD 連結 2](https://hackmd.io/@vVHC1jffQWuFZd6HVu7sFg/BkgDazEEWe) |
| 🎬 課堂影片 | [元智大學課程](https://e.lifelong.yzu.edu.tw/course/53) |

---

## Week 2｜單元二：Web 小工具快速上線

### 📋 課程主題
**從 IPO 邏輯驗證到 Git 版本控管**

### 🎯 學習重點
- Web 工具型態：靜態網頁 vs 動態網頁
- TDD（測試驅動開發）思維
- Git 與 GitHub Desktop：版本控制的時光機
- 部署到 GitHub Pages

### 📎 課程資源

| 項目 | 連結 |
|------|------|
| 📝 操作手冊 | [HackMD 連結 1](https://hackmd.io/@vVHC1jffQWuFZd6HVu7sFg/rJIm7HXHWl) |
| 📝 操作手冊 | [HackMD 連結 2](https://hackmd.io/@vVHC1jffQWuFZd6HVu7sFg/rkev1IXrbx) |
| 🎬 課堂影片 | [元智大學課程](https://e.lifelong.yzu.edu.tw/course/56) |
| 📚 補充資料 | [Git 教學網站](https://web.archive.org/web/20230518032601/https://backlog.com/git-tutorial/tw/) |

---

## Week 3｜單元三：Google Apps Script (GAS)

### 📋 課程主題
**GAS 介紹與應用、連結 VS Code**

### 📎 課程資源

| 項目 | 連結 |
|------|------|
| 📝 操作手冊 | [HackMD 連結](https://hackmd.io/@vVHC1jffQWuFZd6HVu7sFg/r1H78_hB-e) |
| 🎬 課堂影片 | [元智大學課程](https://e.lifelong.yzu.edu.tw/course/57) |

---

## Week 4｜單元四：LINE Bot 助理

### 📋 課程主題
**行動指揮中心 — 打造你的 LINE Bot 助理**

### 📎 課程資源

| 項目 | 連結 |
|------|------|
| 📝 操作手冊 | [HackMD 連結](https://hackmd.io/@vVHC1jffQWuFZd6HVu7sFg/HykF71LLZx) |
| 🎬 課堂影片 | [元智大學課程](https://e.lifelong.yzu.edu.tw/course/58) |
| 📺 補充影片 | [YouTube 影片](https://www.youtube.com/watch?v=8p-uWpy-Uh4) |

---

## Week 5｜單元五：爬蟲技術介紹

### 📋 課程主題
**網頁爬蟲技術入門**

### 📎 課程資源

| 項目 | 連結 |
|------|------|
| 📝 操作手冊 | [HackMD 連結](https://hackmd.io/@vVHC1jffQWuFZd6HVu7sFg/BkcpQ7kvbx) |
| 🎬 課堂影片 | 待放 |

---

## 🔑 核心概念

### Vibe Coding 開發流程
```
Spec (I/O) → AI (TDD) → Validate → Git → Deploy
```

### IPO 思維框架
- **I (Input)**: 輸入什麼？
- **P (Process)**: 處理邏輯？
- **O (Output)**: 輸出什麼？
- **AC**: 驗收標準 3×3# Winter-Break-vibe-coding-course
寒假 Vibe Coding 課程資料整理
